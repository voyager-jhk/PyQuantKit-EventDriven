...
class DataHandler:
    ...
    def __init__(self, events_queue: deque, symbol_list: list):
        ...
        self.latest_symbol_data = {s: {} for s in symbol_list} # Stores latest bar for each symbol
...
class HistoricCSVDataHandler(DataHandler):
    ...
    def _load_csv_data(self):
        for symbol in self.symbol_list:
            file_path = f"{self.csv_dir}/{symbol}_1d.csv" # Assuming 'SYMBOL_1d.csv' format
            try:
        ...
        for symbol in self.symbol_list:
            try:
                ...
                bar_data = {
                    ...
                    'close': row['Close'], 'volume': row['Volume'], 'adj_close': row.get('Adj Close', row['Close'])
                }
                self.latest_symbol_data[symbol] = bar_data
                market_event = MarketEvent(timestamp=timestamp, symbol=symbol, data=bar_data)
                ...
                print(f"Error accessing data for {symbol} at {timestamp}: {e}. Row: {row}")
